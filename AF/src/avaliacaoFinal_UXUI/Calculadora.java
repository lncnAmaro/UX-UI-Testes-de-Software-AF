...
public class Calculadora {
	 ...
	 * @param a primeiro operando
	 * @param b segundo operando
	 * @return o resultado da divisão de {@code a} e {@code b}, se a condição {@code b} diferente de 0. Se {@code b} for igual a zero, será lançada uma exceção.
	 */
	public int dividir (int a, int b) {
		if (b == 0) {
			throw new IllegalArgumentException("Divisão por zero não é permitida"); // Exceção lançada se 'b' for igual a 0
		}
		return a / b;
	}
	...
	public int calcular (int a, int b, String operador) {
		...
		case "/" -> dividir(a, b);
		default -> throw new IllegalArgumentException("Operador inválido: " + operador); // Exceção
		};
	}
}
